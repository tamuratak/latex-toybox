...
We load `viewer.html` of PDF.js in `iframe` in [Webview](https://code.visualstudio.com/api/extension-guides/webview) of VS Code through a local Web [server](https://github.com/tamuratak/latex-toybox/blob/main/src/components/server.ts). The Webview's content including the `iframe` tag is generated by [PdfViewerPanelService.getPDFViewerContent](https://github.com/tamuratak/latex-toybox/blob/bd46389c62c818e064961644b71f6a2917242008/src/components/viewerlib/pdfviewerpanel.ts#L150), which loads [pdfviewerpanel.js](https://github.com/tamuratak/latex-toybox/blob/main/resources/pdfviewerpanel/pdfviewerpanel.js). On the browser, we directly load `viewer.html`. This architecture enables us to load the same `viewer.html` both on VS Code and on the browser.
We tweak DOMs on `viewer.html` through [editviewerhtml.ts](https://github.com/tamuratak/latex-toybox/blob/main/viewer/components/editviewerhtml.ts).

We control the PDF viewers on both the tab and the browser through WebSockets in the same way. The handler on the server side is defined as [Viewer.handler](https://github.com/tamuratak/latex-toybox/blob/bd46389c62c818e064961644b71f6a2917242008/src/components/viewer.ts#L258).

We provide additional features by setting up new event listeners in `latextoybox.ts` for DOM objects in `viewer.html`. We do not and should not override functions defined by PDF.js.
